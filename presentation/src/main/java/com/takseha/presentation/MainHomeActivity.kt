...
import com.takseha.presentation.exception.ExceptionToFailureMapper
...
class MainHomeActivity : AppCompatActivity() {
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.mainHomeUiState.collect { state ->
                    when (state) {
                        ...
                        is UiState.Error -> showErrorState(state.alertMsg, state.data ?: MainHomeUiState())
                        else -> Unit
                    }
                }
            }
    ...
    private fun setupListeners() {
        try {
            ...
            lifecycleScope.launch {
                viewModel.mapFailureToUiState(ExceptionToFailureMapper.map(e))
            }
        }
    }
